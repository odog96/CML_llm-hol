...
HTML_DIR = 'reference_html'

# Clean up the directory if it exists, if not then create it
if os.path.exists(HTML_DIR):
    files = glob.glob(f"{HTML_DIR}/*")
    for f in files:
        os.remove(f)
else:
...
def main():
    ...
    async def main():
        ...
        # Enable queue before mounting the Gradio app
        combined_demo.queue()
        
        # Integrate with FastAPI to serve HTML files
        gr_app = gr.mount_gradio_app(app=FastAPI(), blocks=combined_demo, path="/")
        
        print("Gradio app ready")
        
        @gr_app.get("/view-html/{html_filename}")
        def view_html(html_filename: str):
...
def generate_html_file(chunk_text):
...
def get_responses(message, history, model, temperature, token_count, vector_db):
    ...
    if model == "Local Mistral 7B":
        ...
        if vector_db == "None":
            ...
            html_file_path = generate_html_file(context_chunk)

            file_url = f'/{html_file_path}'  # Assuming Gradio will serve from the current directory
            html_link = f'<a href="{file_url}" target="_blank">View Referenced Chunk</a>'
            
            # Add reference to specific document in the response
            response = f"{response}\n\n For additional info see: {html_link}"
